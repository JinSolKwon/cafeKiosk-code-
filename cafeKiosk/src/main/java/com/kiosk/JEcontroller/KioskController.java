...
public class KioskController {
	...
	String registPost(MemberVo member, String month, String day, RedirectAttributes rttr, HttpSession session)
			throws Exception {
		String namePattern = "^[ㄱ-ㅎ가-힣a-zA-Z]*$";
		if (member.getName() == null || member.getName() == "" || member.getBirthYear() == "" || month == ""
				|| day == "") {
	...
	String registCheck(MemberVo member, String month, String day, RedirectAttributes rttr) throws Exception {
		...
		if (member.getPhone() == null || member.getPhone().length() != 13
				|| !(Pattern.matches(phonePattern, member.getPhone()))) {
	...
	String login(String phone, HttpSession session, RedirectAttributes rttr) throws Exception {
		String phonePattern = "^01(?:0|1|[6-9])-(?:\\d{3}|\\d{4})-\\d{4}$";
		if (phone.equals(null) || phone == "" || phone.isEmpty() || phone.length() != 13
