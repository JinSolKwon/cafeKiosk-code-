...
<body>
	...
	<div id="manageMain">
		...
		<form name="quitMemberForm">
			...
			<table class="table table-hover" id="example-table-1">
				...
				<tbody>
					<c:forEach var="quitMember" items="${quitMemberList}">
					<tr>
						<td onclick="event.cancelBubble=true">
							<input type="checkbox" class="form-check-input" name="reportChkBxRow" value="${quitMember.num}">
						</td>
						<td style="cursor:pointer;">
							<c:out value="${number}"/>
							<c:set var="number" value="${number + 1}"/>
						</td>
						<td class="hidden-col">${quitMember.num}</td>
						<td style="cursor:pointer;">${quitMember.name}</td>
						<td style="cursor:pointer;">${quitMember.phone}</td>
						<td style="cursor:pointer;">${quitMember.birthYear}.${quitMember.birth}</td>
						<td style="cursor:pointer;">
							<fmt:formatDate value="${quitMember.regdate}" pattern="yyyy-MM-dd"/>
						</td>
					</tr>
					</c:forEach>
				</tbody>
		  ...
		  <ul class="pagination justify-content-center">
		    <c:if test="${startPage > pageBlock}">
			    <li class="page-item disabled">
		      		<a class="page-link" href="<c:url value="/managerPage/deleteMemberControl?pageNum=${startPage - pageBlock}"/>">Previous</a>
		    	</li>
		    </c:if>
		    <c:forEach var="i" begin="${startPage}" end="${endPage}">
		    	<li class="page-item"><a class="page-link" href="<c:url value="/managerPage/deleteMemberControl?pageNum=${i}"/>">${i}</a></li>
		    </c:forEach>
		    <c:if test="${endPage < pageCount}">
		    	<li class="page-item">
		      		<a class="page-link" href="<c:url value="/managerPage/deleteMemberControl?pageNum=${startPage - pageBlock}"/>">Next</a>
		    	</li>
		    </c:if>
		  </ul>
		  </c:if>
	...
	var url = "deleteQuitMember";    // Controller로 보내고자 하는 URL
		...
		var chk = Swal.fire({
				...
				$.ajax({ 
	                ...
	                success: function(jdata){
	                    if(jdata = 1) {
	                        alert("삭제 성공");
	                        location.replace("deleteMemberControl") //list 로 페이지 새로고침
	                    }
	                    else{
	                        alert("삭제 실패");
	                    }
	...
	function restoreValue(){
		var url = "restoreMember";    // Controller로 보내고자 하는 URL
		var valueArr = new Array();
	    var list = $("input[name='reportChkBxRow']");
	    for(var i = 0; i < list.length; i++){
	        if(list[i].checked){ //선택되어 있으면 배열에 값을 저장함
