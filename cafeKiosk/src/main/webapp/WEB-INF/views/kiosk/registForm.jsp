...
<body>
	<div class="container-main">
		<div class="header-main">
			<img alt="cafeCarp"
				src="<c:url value="/resources/img/cafeCarp_main.png" />">
		</div>
		<form id="registForm" method="post">
			<div class="main-regist">
				<div class="main-regist1">
					...
					<input id="phone" name="phone" type="text" maxlength="13"
						value="${member.phone}" placeholder="'-'를 제외한 숫자 11자리 입력 " />
					<button id="phoneCHK" type="submit"
						onclick="form.action='<c:url value="/kiosk/phoneCheck" />'">중복체크</button>
				</div>
				<div class="main-regist2">
						...
						<select id="year" name="birthYear">
							...
							<c:forEach var="i" begin="0" end="100" step="1">
								...
								<option value="${optionYear}"
									<c:if test="${!empty member.birthYear && optionYear == member.birthYear}">selected="selected"</c:if>>${optionYear}</option>
							</c:forEach>
						</select>
					</div>
					<div class="main-regist3-3">
						<select id="month" name="month">
							...
							<c:forEach var="i" begin="1" end="12" step="1">
								<option value="${i}"
									<c:if test="${!empty month && i == month}">selected="selected"</c:if>>${i}</option>
							</c:forEach>
						</select>
					</div>
					<div class="main-regist3-4">
						<select id="day" name="day">
							...
							<c:forEach var="i" begin="1" end="31" step="1">
								<option value="${i}"
									<c:if test="${!empty day && i == day}">selected="selected"</c:if>>${i}</option>
							</c:forEach>
						</select>
					</div>
				</div>
			...
			<div class="footer-btn-regist">
				<div class="footer-btn-regist1">
					<button type="button"
						onclick="location.href='<c:url value="/kiosk/main" />'">가입취소</button>
				</div>
				<div class="footer-btn-regist2">
					<button id="subBtn" type="submit"
						onclick="form.action='<c:url value="/kiosk/regist" />'"
