	...
	<div id="container">
		...
		<div class="main-orderResult">
			<table id="table-orderResult">
				<tr class="th-orderResult">
					<td class="th-orderResult1" colspan="2">메뉴</td>
				...
				<c:forEach items="${orderResultList}" var="orderOne"
					...
					<tr class="tr-orderResult1">
						<td class="td-orderResult1"><b>${orderOne.getMenu()}</b> <span><c:if
									test="${not empty orderOne.getTemperature()}">( ${orderOne.getTemperature()} )</c:if></span>
						</td>
						<c:choose>
		...
		<div class="side-orderResult">
			...
			<div id="side-orderResult2">
				<div id="side-orderResult2-1">
					<button onclick="history.go(-1)">취 소</button>
				</div>
				<div id="side-orderResult2-2">
					...
					<button id="subBtn"
						onclick="location.href='<c:url value="${nextUrl}" />'">결
						제</button>
				</div>
			</div>
		</div>
