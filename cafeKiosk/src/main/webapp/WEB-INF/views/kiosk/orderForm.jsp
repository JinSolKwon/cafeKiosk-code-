...
<link href="<c:url value="/resources/css/userOrder.css" />"
...
<body>
	<c:if test="${!empty sessionScope.member}">
		<c:set var="length"
			value="${fn:length(sessionScope.member.getPhone())}" />
		<c:set var="phone2"
			value="${fn:substring(sessionScope.member.getPhone(), length-4, length-2)}" />
		<c:set var="user" value="${phone2}**님" />
		<fmt:formatNumber var="point" pattern="#,###"
			value="${sessionScope.member.getPoint()}"></fmt:formatNumber>
		<c:set var="point2" value="${point}P" />
	</c:if>
	<div id="container">
		<div class="header-orderForm">
			<div class="header-orderForm1">
				<div class="header-orderForm1-1">
					<button type="button"
						onclick="location.href='<c:url value="/kiosk/scroll?type=A" />'">◀
					</button>
				</div>
				<div class="header-orderForm1-2">
					<ul id="cateUl-orderForm">
						<c:forEach items="${sessionScope.cateList}" var="cateOne"
							varStatus="status">
							<li id="cateLi-orderForm${satus.count}"><a
								href="<c:url value="/kiosk/order?num=${cateOne.getNum()}" />">${cateOne.getCategory() }</a></li>
						</c:forEach>
					</ul>
				</div>
				<div class="header-orderForm1-3">
					<button type="button"
						onclick="location.href='<c:url value="/kiosk/scroll?type=N" />'">▶</button>
				</div>
			</div>
			<div id="header-orderForm2">
				<div id="header-orderForm2-1">${user}</div>
				<div id="header-orderForm2-2">${point2}</div>
				<div id="header-orderForm2-3">
					<button onclick="mainBack();">
						MAIN<br>Go
					</button>
				</div>
			</div>
		...
		<div class="main-orderForm">
			<c:forEach items="${menuList}" var="menuOne">
				<a
					onclick="modalOpen('${menuOne.MENU}','${menuOne.PRICE}','${menuOne.SAVE_NAME}','${menuOne.CATEGORY_NUM}','${menuOne.TYPE}')">
					<div class="main-orderForm1">
						<c:if test="${empty menuOne.SAVE_NAME}">
							<div class="main-orderForm1-image">
								<img alt="${menuOne.MENU}"
									src="<c:url value="/display?saveName=noimage.gif" />">
							</div>
						</c:if>
						<c:if test="${!empty menuOne.SAVE_NAME}">
							<div class="main-orderForm1-image">
								<img alt="${menuOne.MENU}"
									src="<c:url value="/display?saveName=${menuOne.SAVE_NAME}" />">
							</div>
						</c:if>
						<div class="main-orderForm1-txt">
							<div class="main-orderForm1-name">${menuOne.MENU}</div>
							<div class="main-orderForm1-price">
								<fmt:formatNumber var="price" pattern="#,###"
									value="${menuOne.PRICE}" />${price}
								원
							</div>
						</div>
					</div>
		...
		<div class="side-orderForm">
					...
					<c:forEach items="${sessionScope.orderList}" var="orderOne"
						...
						<div class="side-orderForm1-1">
							<div class="side-orderForm1-2">
								...
								<div class="side-orderForm1-2-2">
									<c:if test="${orderOne.getType() == 1}">( ${orderOne.getTemperature()} )</c:if>
								</div>
								<div class="side-orderForm1-2-3">
									<div class="side-orderForm1-2-3-1">
										<button type="button"
											onclick="location.href='<c:url value="/kiosk/orderDel?num=${status.index}" />'">X</button>
									</div>
									<div class="side-orderForm1-2-3-2">
										<fmt:formatNumber var="orderPrice" pattern="#,###"
											value="${orderOne.getPrice()}" />
							...
							<c:if test="${orderOne.getType() == 1}">
								<div class="side-orderForm1-3">┗ 사이즈 :
									${orderOne.getBeverageSize()}</div>
								<c:choose>
			...
			<div id="side-orderForm2">
				<div class="side-orderForm2-1">
					<c:if test="${not empty sessionScope.orderCount}">
						<c:set var="orCount" value="수량  : ${sessionScope.orderCount}" />${orCount}</c:if>
				</div>
				<div class="side-orderForm2-2">
					<c:if test="${not empty sessionScope.orderTotal}">
						<fmt:formatNumber var="orderTotal1"
							value="${sessionScope.orderTotal}" pattern="#,###" />
			...
			<div id="side-orderForm3">
				<button id="subBtn"
					onclick="location.href='<c:url value="/kiosk/orderResult"/>'"
					disabled="true">결 제 하 기</button>
			</div>
		</div>
	</div>
	<div class="modal" id="optionModal" role="dialog"
					...
					<div class="modal-header">
						...
						<div class="modal-header2">
							<div class="modal-header2-1">
								<span id="modal-name"></span> <input type="hidden" name="menu"
									id="modal-name-hidden" />
							</div>
						...
						<div class="modal-header3">
							<label><input type="radio" name="temperature" value="ICE"
								checked="checked" />ICE</label> <label><input type="radio"
								name="temperature" value="HOT" />HOT</label>
						</div>
					</div>
					<div class="modal-body">
						<div class="main-optionForm1">
							<div class="main-optionForm1-1">
								<span>사이즈</span>
							</div>
							<div class="main-optionForm1-2">
								<label><input type="radio" name="beverageSize" value="M"
									checked="checked" />M</label>&nbsp;&nbsp;&nbsp;&nbsp; <label><input
									type="radio" name="beverageSize" value="L" />L(+500)</label>&nbsp;&nbsp;&nbsp;&nbsp;
								<label><input type="radio" name="beverageSize"
									value="XL" />XL(+1000)</label>
							</div>
						</div>
						<div class="main-optionForm1">
							<div class="main-optionForm1-1">
							...
							<div class="main-optionForm1-2">
								<label><input type="radio" name="whipping" value="N"
									checked="checked" />없음</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <label><input
									type="radio" name="whipping" value="Y" />휘핑추가</label>
							</div>
						</div>
						<div class="main-optionForm1">
							<div class="main-optionForm1-1">
							...
							<div class="main-optionForm1-2">
								...
								<input size="5px" type="text" name="syrub" value="0"
						...
						<div class="main-optionForm1">
							...
							<div class="main-optionForm1-2">
								...
								<input size="5px" type="text" name="shot" value="0"
									readonly="readonly" style="text-align: center;" />
						...
						<div class="main-optionForm1">
							...
							<input type="hidden" name="price" id="modal-price-hidden" /> <input
								type="hidden" name="origiPrice" id="modal-origin-price-hidden" />
